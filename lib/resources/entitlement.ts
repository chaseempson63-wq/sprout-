...
//
//   getAppleSub(user)      — the identity bridge. The app sets the RevenueCat
//                            App User ID to the Apple `sub`, so the web must
//                            match on the SAME Apple sub, NOT the Supabase UUID.
...
export async function checkEntitlement(appleSub: string | null): Promise<EntitlementResult> {
  ...
  const entitlementId = process.env.REVENUECAT_ENTITLEMENT_ID ?? "resources";
