...
export const metadata: Metadata = {
  ...
  description:
    "Worksheets built and shared by Sprout parents, a chat to ask and swap ideas, and updates from the Sprout team.",
};

export default async function CommunityPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  const { tab } = await searchParams;
  const initial: CommunityTab = tab === "chat" || tab === "announcements" ? tab : "worksheets";
