...
- **Pricing (updated 2026-06-11, see the V1 build spec section)**: testing **$15/week and $30/month**, charged **in-app via Apple IAP** (applying for the Apple Small Business Program, 15% not 30%). **Hard paywall** after a long (~30-step, 5-10 min) onboarding that does the selling. No free trial. First week's compiled view exportable-and-yours-forever (shareability = marketing). The old $29/mo / $249/yr placeholder is retired.
- **Multi-child from day one**: parent → kids[], inputs tagged per-kid or "all," timeline per-kid, flat per-family pricing
- **Scope (v0.1)**: relentlessly tight — no curriculum, no lesson plans, no rego paperwork as the headline (latent benefit only), no AI synthesis in the app loop, no data sold or shared or trained on, no community features. (Note: V1 IS a native mobile app, built in Rork. The earlier "no native app / web responsive" fence is retired 2026-06-11.) **In scope**: calendar with deadlines, scheduled activities, voice/photo/text documentation, per-kid timeline, weekly view, kid-facing visual view (accessed from parent's session for now — separate kid logins TBD), **streaks + gamification + progress rings (added 2026-06-03 — accountability mechanics are working across consumer apps, this is the fitness-app pattern Sprout is built on)**.

## V1 build spec (locked 2026-06-11)

The shipping spec for the app. When this conflicts with older dated notes below (the $29 pricing placeholder, the "no native mobile app" fence, the "CC builds a thin MVP in this repo" framing), this wins.

**Who builds it:** Chase builds the app solely in **Rork** (rork.com, AI builder, Expo / React Native, GitHub-syncs, Supabase-capable). It is a separate native app, NOT this Next.js project (which stays the marketing/landing site on Vercel). **CC's role:** planning, strategy, A/B testing, content, FB posts, funnel. Not app code.

**The loop (Cal AI simple in, proof-of-work out):** parent snaps a photo of what the kid did, the app prompts for a short description (voice-to-text or typed), the photo + description + a tapped subject/activity chip land on that kid's timeline, and the timeline rolls up into weekly + monthly reports AND visual dashboards (rings, bars, time-per-subject, the fitness-app payoff). Capture stays "photo, bang"; the subject chip rides inside the description step so the dashboards get clean data with no AI classification (keeps the no-AI / privacy line intact). [Open, low-stakes: tap-chip vs auto-infer the subject. Defaulting to tap-chip.]

**In V1:** email auth (Supabase) · onboarding (Chase owns it, ~30 steps, 5-10 min, high-investment quiz-funnel into the paywall) · capture (photo + voice/text + subject chip) · per-kid timeline · weekly + monthly view · visual dashboards · multi-child · hard paywall.

**Kid view:** not a separate mode or login. The same view. Parent hands the kid the phone on the weekly report ("look what we did"). Bonus: the kid picks up the documenting habit. Do not build a separate kid app.

**Pricing (supersedes the $29 placeholder):** $15/week and $30/month, test these to start. Hard paywall after the long onboarding. Payments **in-app via Apple IAP** (not web), deliberate: native App Store purchase = trust, and it keeps the post-onboarding funnel seamless with no browser kick-out. Applying for the **Apple Small Business Program** (15% not 30% on first $1M) ASAP; the cut is a tax-deductible expense and not a concern at this stage. Wire IAP through **RevenueCat**, not raw StoreKit. (The older "web-first Stripe to dodge Apple 30%" note applies to the affiliate / web purchase path, not the app paywall.)

**Regulation:** untouched. It is documentation, not compliance. No "meets standards" claim anywhere in marketing. Latent truth: it does ~80% of the proof-of-work lifting, so when she does need to show records most of it is already there and she just fills gaps. Never promised in marketing.

**Offer (parked, not a V1 build priority):** "this price today only" urgency + access to a 1,000+ printables library, filterable by kid age, exportable in-app. The limited-time freebie hook. Logged for later.

**Positioning:** homeschool is the demand-led wedge; the product is built parent-generic (parent → kids → captures) and broadens to parenting / family education tracking after. Homeschool lives in copy / onboarding / App Store listing, not the schema. See memory `sprout-build-rork-role-split`.

## Stack

Next.js 16 (App Router, Turbopack) · React 19 · TypeScript · Tailwind v4 · shadcn/ui · Geist Sans · Lucide. Planned (add when needed): Supabase (AU region) · Resend · Stripe. **No LLM API in the app loop** — Sprout organises and visualises, doesn't generate. Voice memos may be transcribed via on-device or third-party transcription only (no training data shared). If that ever changes, it's a positioning decision, not a technical one. **The V1 app is NOT in this repo** — it is a separate native build in Rork (Expo / React Native, Supabase, RevenueCat for IAP). This Next.js project is the marketing/landing site only.
...
- **The product (V1 build starting 2026-06-11, built in Rork by Chase — see "V1 build spec" above).** The app is a separate native (Expo) build in Rork, NOT this repo, and NOT built by CC. This Next.js codebase remains landing-only. Once the app ships it unblocks the UGC/affiliate lane (creators need something real to film, plus an App Store listing for trust) and gives warm contacts something to actually use. The fence: ship a usable slice, don't gold-plate it.
