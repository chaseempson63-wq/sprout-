...
- **Acquisition motion (baseline 2026-05-28; sprint day-12 update 2026-06-03 below)**: a multi-lane distribution stack, all pointing at the waitlist. (1) **Sprout TikTok + Instagram** — organic ~3x/day, niche memes swung Sprout's way + typographic carousels. (2) **Facebook Page** — live, posting, auto-cross-post from Sprout IG. (3) **Personal-account value funnel (Chase + Alice)** — both inside the homeschool groups; gf posts one high-value giveaway on the week's hot topic, conversation moves to DMs, value delivered first, waitlist link as soft follow-up. (4) **UGC affiliate (planned, blocked on the MVP existing)** — find moms who already believe, app shown 3-5s of natural use (not a pitch), soft CTA in the description, their affiliate link = 20% off (first 3mo only) + 20% creator fee; zero upfront promo spend. (5) **YouTube (planned)** — AI homeschool content with Sprout as the channel sponsor. Warm parent conversations run alongside for value-validation (not site iteration). gf is now a genuine collaborator on the distribution side.
  ...
  **Sprint day-12 update (2026-06-03):**
  ...
  - **Lane 5 (YouTube)** stagnant — probably another week at least.
  - **The FB → waitlist gap to watch:** 50 inbound from Alice's first drop, but waitlist count flat. People want the freebie, not (yet) Sprout. Back-end push design needs to bridge this — at minimum, the freebie itself should mention Sprout + the waitlist in one line (footer, last page) before this lane scales further.
...
**Honest co-founder read (2026-06-03): closer to 6–7/10. Real signal, but on a side door.** The 50 inbound proves freebie appetite (people want the homeschool PDF). It does NOT prove Sprout appetite. The waitlist count remains flat through the same period. The conversion from "got the freebie" → "joined the Sprout list" → "would pay $29/mo" is three hops, all unvalidated. Zero "yes I'd pay" replies after weeks of warm contact. The structural validation gates from 5/20 (willingness-to-pay, behavior-adoption, founder-in-niche) are still open — none have closed yet, the new signal sits on a different wound (affordability/free-resources) than Sprout's core wound (parental anxiety). The play could absolutely work — funnels often need the lead-magnet door to surface the real prospects — but **the freebie → Sprout-waitlist bridge is the load-bearing build** before this lane scales further. Watch the waitlist conversion rate from the FB inbound, not the inbound count itself.
