...
- **Acquisition motion (baseline 2026-05-28; sprint day-12 update 2026-06-03 below)**: a multi-lane distribution stack, all pointing at the waitlist. (1) **Sprout TikTok + Instagram** — organic ~3x/day, niche memes swung Sprout's way + typographic carousels. (2) **Facebook Page** — live, posting, auto-cross-post from Sprout IG. (3) **Personal-account value funnel (Chase + Alice)** — both inside the homeschool groups; gf posts one high-value giveaway on the week's hot topic, conversation moves to DMs, value delivered first, waitlist link as soft follow-up. (4) **UGC affiliate (planned, blocked on the MVP existing)** — find moms who already believe, app shown 3-5s of natural use (not a pitch), soft CTA in the description, their affiliate link = 20% off (first 3mo only) + 20% creator fee; zero upfront promo spend. (5) **YouTube (planned)** — AI homeschool content with Sprout as the channel sponsor. Warm parent conversations run alongside for value-validation (not site iteration). gf is now a genuine collaborator on the distribution side.
  ...
  **Sprint day-12 update (2026-06-03):**
  - **Lane 3 (FB groups) is the strongest signal in the stack so far.** Alice's first post in the groups yesterday → 50 inbound DMs, 30 in the last 24h. Freebie going out; back-end Sprout push is the next build. Doubling down on this lane.
  ...
  - **Lane 1 + 2 (Sprout IG/TT + FB Page)** still warming. 14 IG posts shipped, 6 drafts queued. Same on TikTok. FB Page auto-cross-post from IG live. Reach throttled by algos still warming. TikTok blocked from converting until 1k followers (no link-in-bio).
  - **Lane 5 (YouTube) — concrete plan now, queued for ~2 weeks out (~mid-June, post-MVP).** 5-10 minute educational kids videos. Sprout is the **self-sponsor** (no out-of-pocket partnership cost, full creative control): 30-sec baked-in Sprout ad per video + Sprout link in description. Each long-form video gets clipped into Shorts for multi-surface distribution. Why YT now: kids ed is a fast-growing niche, the parent (Sprout's buyer) is in the room when the kid watches, and self-sponsorship turns a channel into a permanent recurring promo slot. Open watch-items pre-launch: (a) production capacity — a good 5-10 min video is ~4-8 hours of work, doesn't yet have a slot in his weekly cadence; (b) wedge — kids ed niche is large but crowded (Ms Rachel, CoComelon, Khan Academy Kids), needs a specific angle ("kids ed from the homeschool mom POV" is the candidate wedge).
  - **New lane 8: Meta paid ads — concrete plan, gated on MVP-shipped + working A→Z.** Format: 10-15 static creatives (cheaper to produce + iterate than video, faster to A/B, clearer to call out ICP + pain point in one frame). Budget: $30-50/day for 1 week (~$210-350 total). Goal: test whether cold paid traffic converts at all — proof of concept for the full economic loop, not a scale spend. Open watch-items: (a) **the 7-day window can't measure profitability** — Sprout's 7-day trial means signup-to-paid only happens after day 7, so the test can only measure CTR, CPC, signup rate, and trial activation; plan a follow-up scale window to measure to-paid conversion before killing creative based on week-1 economics; (b) **the 10%+ onboarding conversion target is optimistic** for cold paid (industry typical 2-5%); better framing is "what makes the test informative" (e.g., 100+ signups to find a creative winner) than a binary conversion threshold.
  - **Sprout back-end push deliberately deferred.** First drop was pure value to build trust with group admins and stay under the radar. The Sprout/waitlist mention gets added later once the account has authority in the groups. The flat-waitlist-vs-50-inbound is not a leak — it's the planned trust-build phase. Conversion test runs after the Sprout push is added.
...
**Progress (as of 2026-06-03, ~day 12):** Waitlist still flat (~4, ≈no change since 5/28 — IG/TT reach throttled, no link-in-bio yet on TT) · 14 IG posts shipped on Sprout brand account + 6 queued, same on TikTok, FB Page auto-cross-posting · **Alice's FB groups drop: 50 inbound DMs, 30 in last 24h** (freebie going out, Sprout back-end push is the next build) · Homeschool-with-Charlie faceless IG started 6/2, rolling out next week · cold-creator outreach list compiled (<30k niche) + 10-min creator onboarding course in build (both gated on MVP) · MVP target end of this week · family network (mom + nana) activating this week · YouTube still stagnant. Monday routine handles weekly accountability.
