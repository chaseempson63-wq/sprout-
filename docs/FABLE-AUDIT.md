...
### 2026-07-06 revision round 3 (founder notes, applied)

**Headlines fixed with the locked approved lines, verbatim and distinct** (the recurring miss, corrected): card 1 "You did more than you think." · card 3 "Where your kid's whole week lives" · card 4 "Everything Mia learned this week, in your pocket". No more SaaS rewordings of one idea. **Card 2:** the whole phone is the camera now — the scan fills the full screen, wide brackets, note pinned to the bottom ("Morning reading" + Talk), Retake/shutter/Library at the very bottom, and Sarah R.'s review floats mid-left in the whitespace. Reads as "I'm scanning my kid's work right now." **Card 6:** rebuilt as three steps down the phone — 01 CAPTURED (the glass capture card with a real photo + camera badge), 02 SORTED (color-coded domain rows), 03 REPORTED (the cream paper report) — each replicated from the real `OnboardingVisual` variants in `app/_components/OnboardingVisuals.tsx`, one short line each ("Ten seconds, snap it." / "Filed into their week, for you." / "The report writes itself."), joined by a dashed flow line, under the founder's suggested headline "10 seconds in. The rest is hands-free." **Flame:** the number now sits centered in the flame's body on cards 1 and 4.

### 2026-07-06 revision round 2 (founder notes, applied)

**The flame is now the app's actual flame.** Found it in `WeeklyRecapCard.swift` (`streakBadge`): SF Symbols `flame.fill`, vertical gradient `#FFC94D → #FF7A2E → #E8401C`, orange glow, the streak number sitting WHITE inside it, label below. Replaced my invented teardrop on cards 1 and 4 with a faithful SF-flame silhouette and that exact treatment. **Card 1:** breathing room added between headline and photos; the flat domain pills replaced with the real garden graph (same varying-height bar style as cards 4 and 6, the 3-2-4-5-1-2 wave). **Card 2:** note is now just "Morning reading" so the shot reads plainly as the book being scanned. **Card 3:** phone shrunk to 900px and three floating cream callouts hover off its edges (Scheduling at the calendar, Planned at the plan rows, Documented at the timeline), positioned so the month name stays visible. **Card 4:** headline is "Mia's Week, Recapped"; garden label and the 6-of-7-days stat removed (14 moments + flame only); all five photos at the bottom in a 3+2 grid. **Card 6, rebuilt as the ROI card the set was missing:** streak/level/achievement gone. Headline "Save 3+ Hours a Week" (the brief's own payoff anchor) over "The documenting and reporting, done for you." The phone shows a "June report ready" banner ("Compiled for you from 52 moments. A record of learning, if anyone ever asks." — the landing page's line) above a week of logs accumulating as proof of work (six hours out in nature, walk to the library, read three chapters, solar system project finished, doubled the pancake recipe, a hut bleeding off the bottom). Card 5 and card 7 untouched this round.

### 2026-07-06 revision (founder notes, applied)

Same seven files, re-rendered in place. What changed: **(1) Real photos.** The five real documentation moments in `native-photos/` (painting, reading, solar-system project, tree climbing, nature journaling) replaced every AI-generated image; a CC0 web sweep for supplements returned cold, sterile stock next to the natives, so the five carry all slots with cross-card reuse — it reads as one family's week. **(2) Empty space killed by bigger visuals**, not text: card 1 fans three photos into a full-frame artifact with day-check circles; card 3's calendar became a real full month with today ring, plan rows, and a third timeline entry bleeding off the phone; card 4's recap grew ~15% and card 7's pillars doubled in presence. **(3) Cream cards on the green canvas** everywhere in-phone (the real app surface, `Palette.cream/creamRaised` from Theme.swift). **(4) Sprout brand (mascot chip + wordmark) top-left of every phone screen.** **(5) The streak is a custom flame with the number inside it** (gold-to-amber SVG, shared `.flame` primitive) on cards 1, 4, and 6 — no more emoji. **(6) Card 2** runs the founder's exact headline ("Capture every moment in ten seconds."), shows the in-app camera mid-capture (scan brackets, shutter, Retake/Library) over the real reading photo, cream note card, review signed just "Sarah R." **(7) Card 4** rebuilt to the real recap card's order — garden graph big at top, moments/days/flame stats, photos at the bottom, mascot top-right — headline now "The Whole Week, on One Card." **(8) Card 7** headline is "Your Data Stays Yours" with the four pillars verbatim from the app's onboarding PrivacyStep (`OnboardingSell.swift`): we don't have your data to sell · AI's never trained on it · no ads in the app · lives on your device, yours forever. Card 5 untouched per the brief.
